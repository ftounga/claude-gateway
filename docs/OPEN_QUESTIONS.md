...
| 3 | ~~**Durée de l'essai : 5 appliqués contre 14 annoncés**~~ **— TRANCHÉ ET LIVRÉ le 2026-09-11 (F-66)** | Le code sert désormais les **14 jours annoncés** (`app.billing.trial-days` = 14), et l'API expose la durée servie pour que les écrans cessent de la réciter. **Le sens de la correction a été renversé par la vérification** : le risque financier d'un essai n'est pas sa **durée** mais son **quota** — un essai coûte au plus ≈ 1,80 $ depuis le décompte pondéré de F-63, quel que soit le style d'usage. Quatorze jours ne consomment donc pas plus que cinq. **Et ce qui fuyait vraiment n'était ni l'un ni l'autre** : le plafond s'opposait au **mois calendaire**, si bien qu'un essai à cheval sur un 1er du mois valait **400 000 jetons ≈ 3,60 $**. SF-66-01 en a fait une **enveloppe unique**, **avant** d'allonger la durée — qui aurait porté la part des essais à cheval de ≈ 13 % à ≈ 47 % | **Rien** — la question est close. Sa suite vit au **point 9** |
...
| 8 | ~~**Montant du supplément par poste, jetons qu'il apporte, et paliers de dégressivité**~~ **— TRANCHÉ le 2026-09-13 par le PO (F-107 §9) et LIVRÉ le 2026-09-13 (F-107 / SF-107-05, `docs/TARIFS.md` §8.2 ; prices à créer par le PO)** : par espace — Forge 39 € (2e-3e, 2 M) / 29 € (4e-6e, 1,5 M) / 19 € (7e+, 1 M), 0 jeton en BYOK ; Vigie 39 € fixe (3 M de synchro). *Texte d'origine :* *(ajouté le 2026-09-11 par F-65)* | Le **mécanisme** est livré et **inerte** : `app.seat.included-seats` = 1 (l'abonnement couvre un poste), `tokens-per-extra-seat` = **0**, `quota-tiers` **vide**, `price-id` **vide** — donc aucun jeton apporté, aucun supplément facturé, quota rigoureusement identique à celui d'avant F-65. La proratisation d'un poste ouvert en cours de mois est tranchée (prorata temporis à la journée, `proration: DAILY`), la réouverture dans le mois aussi (un mois-poste se paie une fois) | Trois valeurs, toutes commerciales : **(a)** le montant mensuel du supplément — il n'existe que chez Stripe, sous un price ID que le PO créera et renseignera dans `app.seat.price-id` ; **(b)** les jetons qu'il apporte (`tokens-per-extra-seat`) — le calibrage **recommandé** par `STRATEGIE-TARIFAIRE.md` §6 est « le même nombre de tokens par euro que le plan de base », mais le chiffre appartient au PO ; **(c)** les paliers de dégressivité (`quota-tiers` ici, price à paliers chez Stripe) — un consultant à six missions n'acceptera pas six fois le supplément plein, et **les deux grilles doivent être tenues alignées par le PO** : le dépôt ne peut pas le vérifier (même régime que le point 4) |

| 9 | **Dimensionnement de l'essai : 200 000 jetons suffisent-ils ?** *(ajouté le 2026-09-11 par F-66)* | `app.quota.trial-tokens` = **200 000**, soit ≈ **1,80 $** de coût fournisseur au plus (9,00 $/M, §7 point 7) — et, depuis F-66, **une seule fois pour tout l'essai**, quelle que soit sa position dans le calendrier. Le mécanisme est donc sain : ce qui reste est un **calibrage commercial** | 200 000 jetons ne représentent que **quatre à dix tours de Forge** (20 000 à 50 000 jetons par tour) — c'est peut-être trop court pour qu'un essai convainque. **500 000 coûteraient ≈ 4,50 $ par essai** ; 1 M, ≈ 9 $. F-66 s'est **interdit** de trancher : le chiffre appartient au PO, et l'allonger est un arbitrage entre coût d'acquisition et taux de conversion, pas une décision technique. La durée, elle, est désormais gratuite à rallonger — c'est le quota qui porte le coût |
...
**Effet du non-traitement** : aucun sur le code. Sur le commerce, un effet net — les points 1-2
rendent les recharges **invendables autrement qu'à l'aveugle**, le client ne découvrant leur prix
qu'à la page de paiement. *(Le point 3, « promesse publique tenue à 36 % », est refermé depuis le
2026-09-11 : le code sert les 14 jours annoncés. Sa suite est le point 9, qui ne bloque rien — un
essai trop court se voit au taux de conversion, pas en incident.)*
