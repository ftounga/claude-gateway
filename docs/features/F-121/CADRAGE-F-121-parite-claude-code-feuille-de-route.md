...
| **F-121-12** ✅ **LIVRÉ le 2026-09-26** (SF-121-12, PR #941) | **Prompt système plat** + CLAUDE.md injecté verbatim ; pas de guidance de choix d'outil. | Structurer `buildSystemPrompt` en sections stables (Rôle · Investigation · Style · Outils · Conventions projet), encadrer le CLAUDE.md (préambule SF-120-01), ajouter « quand `explore` vs `bash` vs `task` ». — **Livré** : trois bannières `--- Méthode de travail ---` / `--- Style de réponse ---` / `--- Choix des outils ---` (aucun bloc déplacé, rôle toujours premier — SF-148-02) ; borne de fin `--- Fin des conventions du projet (CLAUDE.md) ---` (les deux bornes, ou aucune) ; section de choix d'outil conditionnée à la panoplie réellement déclarée (SF-39-05), la doctrine SF-150-04 restant dans l'outil `task`. Cache F-134 préservé (littéraux + conditions stables). Mini-spec : `docs/features/F-121/SF-121-12-prompt-systeme-sections.md`. |
| **F-121-13** ✅ **CLOS le 2026-09-26 par F-150** (SF-121-13, vérification de parité) | **Pas de sous-agent Task capable d'agir** (seul `explore` en lecture seule ; le `multiagent` du SDK est coupé et hors voie). | Outil `task` maison : sous-boucle réutilisant `runLoop` avec panoplie complète routée vers le **même runner**, budget déduit du plafond de tour, **seule la synthèse remonte**. Réutilise confirmation/audit/cible RUNNER. Aucun SDK. — **Livré par F-150** (PR #825/#827/#829/#834/#836/#842), isolé par **git-worktree sur le poste** ; les cinq critères sont vérifiés un à un dans `docs/features/F-121/SF-121-13-verification-parite-cloture.md`, et les deux qu'aucun test ne gardait (composition de la panoplie de la sous-boucle ; imputation de sa dépense au tour + plafond de délégations) sont désormais **verrouillés par des témoins** (`AtelierChatServiceTaskTest`). |
