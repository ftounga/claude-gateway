...
| Projet d'archive (source `ARCHIVE`) | « Ce projet n'est pas adossé à un dépôt Git » | 400 |
...
- `git/GitTokenService.java` — mémorisation du vault, publication de `GitVaultRevokedEvent`
- `git/GitVaultRevokedEvent.java`, `git/GitVaultRef.java` *(nouveaux)*
...
- `atelier/agent/McpAccess.java`, `atelier/agent/ManagedVault.java` *(nouveaux)*
...
- `atelier/agent/McpVaultService.java` *(nouveau)* — cycle de vie du vault, écoute de la révocation
...
- `account/AccountService.java` — la suppression de compte passe par `GitTokenService.deleteToken`
  pour que le vault du fournisseur parte avec le jeton (RGPD)
...
- `McpVaultService` : vault créé une seule fois puis réutilisé ; jeton absent → aucun vault ;
