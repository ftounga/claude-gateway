...
|---|---|---|
| **1 · Cache** ✅ | SF-39-01 → 02 | `cache_control` sur consigne système, outils et préfixe d'historique · chargement **paresseux des skills** (descriptions seules, corps à la demande) pour rendre le préfixe stable — livré le 2026-09-06 (PR #216, #217) ; **deux marqueurs et non quatre** (D1), celui de fin de consigne couvrant aussi les définitions d'outils, et la consigne n'annonçant plus qu'un **catalogue** de skills : le préfixe cesse d'être tronqué au fil de l'arborescence, donc d'être invalidé sans erreur par l'ajout d'un fichier au projet |
| **2 · Mémoire** ✅ | SF-39-03 → 04 | Rejeu de la trajectoire (`tool_use` / `tool_result`), borné · reprise de fil indépendante de la sandbox (D5) — livré le 2026-09-06 (PR #218, #219, #220) ; la trajectoire vit désormais **chez nous** (`atelier_messages.tool_trace`, migration **050**) et non plus dans la survie d'une sandbox, ce qui rend la reprise **décidable** : `resume` dit ce que le prochain tour rejouera, `restart` pose une frontière (`workspaces.chat_thread_started_at`, migration **051**) **sans rien supprimer** |
| **3 · Outillage** ✅ | SF-39-05 → 06 | Retrait `list_files` / `search_files` · lecture numérotée et paginée · édition ciblée — livré le 2026-09-06 (PR #221, #222, #223) ; la panoplie suit **ce que la cible sait faire** (en `RUNNER` : `read_file` / `write_file` / `edit_file` / `bash` ; `SANDBOX` inchangée, son sort suivant celui de la cible en SF-39-16), et `edit_file` retire au passage le risque qu'une réponse coupée au plafond réécrive un fichier **tronqué** |
