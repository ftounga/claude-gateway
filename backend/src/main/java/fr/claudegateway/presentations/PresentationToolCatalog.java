...
public class PresentationToolCatalog {
    ...
    public static final String GUIDE = "--- Présentations (PowerPoint) ---\n"
            ...
            + "d'image IA (icônes inventées). Fais du diagramme-as-code et appelle render_diagram : LA "
            + "GATEWAY rend l'image et la dépose dans le projet, puis insère-la (add_picture). "
            + "N'INSTALLE RIEN sur la machine du client (F-142 / SF-142-06) : c'est gratuit, aucun jeton. "
            + "Échec NOMMÉ si le rendu échoue — avec la raison rendue ; FACTUEL (ne dessine que "
            + "l'établi, marque le supposé).\n"
            + "ICÔNES CLOUD OFFICIELLES : pour une architecture cloud soignée avec les VRAIS glyphes de "
            + "service (S3, Lambda, RDS, AKS…), appelle render_diagram avec engine=cloud et une "
            + "DESCRIPTION (spec : nodes typés « aws.rds », groups, edges) — recette dans le skill pptx. "
            + "Rendu par la gateway lui aussi (F-142 / SF-142-07) ; un type inconnu est refusé avec les "
            + "types proches, n'en invente pas.\n"
