 ...
 *
 * <p>Un poste n'a qu'une connexion runner à la fois (un runner par machine ; plusieurs runners
 * simultanés sont hors périmètre). {@link #register} remplace toute connexion précédente.</p>
 */
public interface RunnerRegistry {

    /** Enregistre (ou remplace) la connexion runner d'un poste. */
    void register(RunnerConnection connection);

    /**
     * Retire la connexion d'un poste <b>si</b> celle enregistrée a été ouverte par ce jeton
     ...
     */
    void unregister(UUID hostId, UUID tokenId);

    /**
     * Connexion <b>locale</b> vivante d'un poste (sur ce nœud), pour le routage des messages
     ...
     */
    Optional<RunnerConnection> findLocal(UUID hostId);

    /**
     * Nœud <b>distant</b> hébergeant la socket de ce poste, s'il est connu (F-38 / SF-38-12).
     ...
     */
    Optional<RemoteRunnerNode> findRemote(UUID hostId);

    /**
     * Vrai si un runner est connecté pour ce poste, <b>tous replicas confondus</b> (présence
     * locale ou distante). Alimente le statut exposé à l'utilisateur.
     */
    boolean isConnected(UUID hostId);
}
