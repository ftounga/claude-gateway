...
/**
 * Le registre des promotions reportées faute de poste (F-93 / SF-93-04) : ce qui est reporté est
 * réclamé une fois, au bon projet, et jamais ailleurs.
 */
class PromotionReporteeTest {
    ...
    void claimedOnce() {
        registre.reporter(alice, projet, List.of("cluster atlas"), 2);

        assertThat(registre.estDue(alice, projet)).isTrue();
        PromotionReportee.Report report = registre.reclamer(alice, projet).orElseThrow();
        assertThat(report.elements()).containsExactly("cluster atlas");
        assertThat(report.dette()).isEqualTo(2);
        assertThat(registre.reclamer(alice, projet)).isEmpty();
        assertThat(registre.estDue(alice, projet)).isFalse();
    }
    ...
    void reportsAccumulate() {
        registre.reporter(alice, projet, List.of("cluster atlas", " "), 1);
        Instant first = now.get();
        now.set(first.plus(Duration.ofHours(2)));
        registre.reporter(alice, projet, List.of("cluster atlas", "vpn nord"), 0);
    ...
    @Test
    @DisplayName("isolation : ni un autre utilisateur, ni un autre projet ne réclament le report")
    void isolation() {
        registre.reporter(alice, projet, List.of("cluster atlas"), 0);

        assertThat(registre.reclamer(bob, projet)).isEmpty();
        assertThat(registre.reclamer(alice, autreProjet)).isEmpty();
        assertThat(registre.reclamer(alice, projet)).isPresent();
        registre.reporter(null, projet, List.of("x"), 1);
        assertThat(registre.reclamer(null, projet)).isEmpty();
    }
    ...
    void bounded() {
        for (int i = 0; i < PromotionReportee.MAX_ELEMENTS + 5; i++) {
            registre.reporter(alice, projet, List.of("élément " + i), 0);
        }
        assertThat(registre.reclamer(alice, projet).orElseThrow().elements())
                .hasSize(PromotionReportee.MAX_ELEMENTS);

        UUID premier = UUID.randomUUID();
        registre.reporter(premier, projet, List.of("x"), 0);
        for (int i = 0; i < PromotionReportee.MAX_ENTRIES; i++) {
            registre.reporter(UUID.randomUUID(), projet, List.of("x"), 0);
        }
        assertThat(registre.estDue(premier, projet)).isFalse();
    }
